...
// VENDORED COPY of qs.Ui.KeyboardPanel (Omarchy 4.0.0.alpha) with one
// addition: the card's fill is a property instead of a hard-coded
// Color.popups.background, so the panel can take on the colours of the album
// that is playing. Everything else is upstream, verbatim. Re-sync this file if
// KeyboardPanel changes upstream.
//
...
PanelWindow {
  ...
  property color tintTopLeft: cardColor
  property color tintTopRight: cardColor
  property color tintBottomLeft: cardColor
  property color tintBottomRight: cardColor

  function mixColors(a, b) {
    return Qt.rgba((a.r + b.r) / 2, (a.g + b.g) / 2, (a.b + b.b) / 2, 1)
  }

  readonly property color tintTop: mixColors(tintTopLeft, tintTopRight)
  readonly property color tintBottom: mixColors(tintBottomLeft, tintBottomRight)
  readonly property color tintLeft: mixColors(tintTopLeft, tintBottomLeft)
  readonly property color tintRight: mixColors(tintTopRight, tintBottomRight)
  ...
  BorderSurface {
    ...
    // The tint fills the card, but a Rectangle draws its own border underneath
    // its children, so this has to stop at the border's inner edge or it paints
    // the frame away. Same reason the corner radius is carried through.
    Item {
      anchors.fill: parent
      ...
      readonly property real innerRadius: Math.max(0, card.radius - card.borderLeft)
      ...
      Rectangle {
        ...
        gradient: Gradient {
          ...
          GradientStop { position: 0.0; color: root.tintLeft }
          GradientStop { position: 1.0; color: root.tintRight }
        }
      }
    }
